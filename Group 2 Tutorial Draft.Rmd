...
import os
os.chdir("E:/git/Stats506_private/group_proj/")
import pandas as pd
...
We can create marginal box plots by setting `type = "boxplot"`. 
The parameter `size` specifies the relative size of the main plot to the marginal ones. A size of 5 means that the main plot is 5x wider and 5x taller than the marginal plots.  
...
```{r matlab_marginal2, echo = FALSE, out.width = '70%'}
...
```{python plot1_category, eval = TRUE}
...
```{python plot1_plot_setting, eval = TRUE}
left, width = 0.1, 0.65
...
We can divide the plotting into 3 steps:
...
```{python plot1_script, eval = TRUE, out.width = '85%'}
...
```{python plot2_script_seaborn, eval = TRUE, out.width = '85%'}
...
```{r python_bubble_matplotlib, echo = FALSE, out.width = '75%'}
...
```{r matlab_interactive2, echo = FALSE}
htmltools::includeHTML("./interactive2.html")
```
